...
class ShareRegionProcessor {
    ...
    static func prepContent(_ contentText: String, itemPreviews: [ExtensionItemPreview])
        -> [PostEditingService.PostContentRegion]
    {
        ...
        for preview in itemPreviews {
            if let gifData = preview.gifData, let image = preview.image {
                let region = PostEditingService.PostContentRegion.imageData(
                    image,
                    gifData,
                    "image/gif"
                )
