            ...
            describe("itemForIndexPath(_:)") {
                ...
                beforeEach {
                    let match = AutoCompleteMatch(
                        type: AutoCompleteType.username,
                        range: "test".startIndex..<"test".endIndex,
                        text: "test"
                    )
                    let item1 = AutoCompleteItem(
                        result: AutoCompleteResult(name: "test"),
                        type: AutoCompleteType.username,
                        match: match
                    )
                    ...
                    let item3 = AutoCompleteItem(
                        result: AutoCompleteResult(name: "test"),
                        type: AutoCompleteType.username,
                        match: match
                    )
                    let item4 = AutoCompleteItem(
                        result: AutoCompleteResult(name: "test"),
                        type: AutoCompleteType.username,
                        match: match
                    )
                ...
                context("index path exists") {
                    ...
                    it("returns correct item") {
                        expect(subject.itemForIndexPath(IndexPath(row: 1, section: 0))?.type)
                ...
                context("index path does NOT exists") {
                    ...
                    it("returns nil") {
                        expect(subject.itemForIndexPath(IndexPath(row: 100, section: 0))).to(
                            beNil()
                        )
                    }
                }
            ...
            context("UITableViewDataSource") {
                ...
                describe("tableView(_:numberOfrowsInSection:)") {
                    ...
                    it("returns the correct count") {
                        ...
                        expect(
                            subject.tableView(UITableView(frame: .zero), numberOfRowsInSection: 0)
                        ) == 4
                    }
                }
                ...
                describe("tableView(_:cellForRowAtIndexPath:)") {
                    ...
                    it("returns an AutoCompleteCell") {
                        ...
                        let expectedCell = vc.dataSource.tableView(
                            vc.tableView,
