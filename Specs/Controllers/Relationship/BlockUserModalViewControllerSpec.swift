    ...
    override func spec() {
        describe("BlockUserModalViewController") {
            ...
            describe("initialization") {
                ...
                beforeEach {
                    subject = BlockUserModalViewController(
                        config: BlockUserModalConfig(
                            userId: "666",
                            userAtName: "@archer",
                            relationshipPriority: RelationshipPriority.following
                        ) { _ in }
                    )
                ...
                it("sets its transition properties") {
                    expect(subject.modalPresentationStyle).to(
                        equal(UIModalPresentationStyle.custom)
                    )
                    expect(subject.modalTransitionStyle).to(
                        equal(UIModalTransitionStyle.crossDissolve)
                    )
                }
                ...
                it("is correct when relationship is mute") {
                    subject = BlockUserModalViewController(
                        config: BlockUserModalConfig(
                            userId: "666",
                            userAtName: "@archer",
                            relationshipPriority: RelationshipPriority.mute
                        ) { _ in }
                    )
                    subject.currentUser = currentUser
                    showController(subject)
                    expect(subject.view).to(
                        haveSubview {
                            ($0 as? UILabel)?.text == "Would you like to \nunmute or block @archer?"
                        }
                    )
                }
                ...
                it("is correct when relationship is block") {
                    subject = BlockUserModalViewController(
                        config: BlockUserModalConfig(
                            userId: "666",
                            userAtName: "@archer",
                            relationshipPriority: RelationshipPriority.block
                        ) { _ in }
                    )
                    subject.currentUser = currentUser
                    showController(subject)
            ...
            describe("@muteText") {
                it("is correct") {
                    ...
                    expect(subject.view).to(
                        haveSubview {
                            ($0 as? UILabel)?.text
                                == "@archer will not be able to comment on your posts. If @archer mentions you, you will not be notified."
                        }
                    )
                }
            }
            ...
            describe("@blockText") {
                it("is correct") {
                    ...
                    expect(subject.view).to(
                        haveSubview {
                            ($0 as? UILabel)?.text
                                == "@archer will not be able to follow you or view your profile, posts or find you in search."
                        }
                    )
                }
            }
            ...
            describe("@relationship") {
                func blockButtons(_ relationshipPriority: RelationshipPriority) -> (
                    BlockUserModalViewController, UIButton?, UIButton?
                ) {
                    subject = BlockUserModalViewController(
                        config: BlockUserModalConfig(
                            userId: "666",
