...
class ElloWebBrowserViewController: KINWebBrowserViewController {
    ...
    class func navigationControllerWithBrowser(_ webBrowser: ElloWebBrowserViewController)
        -> ElloNavigationController
    {
        // tell AppDelegate to allow rotation
        AppDelegate.restrictRotation = false
        let xButton = UIBarButtonItem.closeButton(
            target: webBrowser,
            action: #selector(ElloWebBrowserViewController.doneButtonPressed(_:))
        )
        let shareButton = UIBarButtonItem(
            image: InterfaceImage.share.normalImage,
            style: .plain,
            target: webBrowser,
            action: #selector(ElloWebBrowserViewController.shareButtonPressed(_:))
        )
    ...
    func shareButtonPressed(_ barButtonItem: UIBarButtonItem) {
        ...
        let activityVC = UIActivityViewController(
            activityItems: [urlForActivityItem],
...
extension ElloWebBrowserViewController: KINWebBrowserDelegate {
    ...
    func webBrowser(
        _ webBrowser: KINWebBrowserViewController!,
        didFailToLoad url: URL?,
        error: Error!
    ) {
...
extension ElloWebBrowserViewController: WebLinkResponder {
    ...
    func webLinkTapped(path: String, type: ElloURIWrapper, data: String?) {
        ...
        case .discover:
            DeepLinking.showDiscover(
                navVC: navigationController,
                currentUser: ElloWebBrowserViewController.currentUser
            )
